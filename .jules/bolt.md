...
## 2025-05-17 - [Optimized Reading Progress Bar & IntersectionObserver]
**Learning:** Updating 'width' on a scroll listener triggers synchronous layout reflows (Layout Thrashing). Moving to 'transform: scaleX()' allows the browser to handle updates on the compositor thread. Additionally, IntersectionObservers should call 'unobserve()' once an element is visible to reduce ongoing main-thread overhead.
**Action:** Always prefer 'transform' and 'opacity' for high-frequency animations and clean up observers as soon as they've served their purpose.
